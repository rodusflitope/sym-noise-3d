...
def main() -> None:
    ...
    for epoch in range(cfg["train"]["epochs"]):
        ...
        if epoch % 10 == 0:
            save_ckpt(model, run_root, run_name, f"epoch_{epoch:03d}.pt", metadata=ckpt_metadata, ema_state=ema_state)
            
        save_ckpt(model, run_root, run_name, "last.pt", metadata=ckpt_metadata, ema_state=ema_state)
        ...
        if sel < best_loss:
            ...
            ckpt_path = save_ckpt(
                model, run_root, run_name, "best.pt", metadata=ckpt_metadata, ema_state=ema_state
            )
