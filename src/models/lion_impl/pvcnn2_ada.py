...
import torch
import numpy as np
from .third_party.pvcnn import functional as F
...
class BallQuery(nn.Module):
    ...
    @custom_fwd(**_AMP_KW, cast_inputs=torch.float32)
...
def create_mlp_components(in_channels, out_channels, classifier=False, dim=2, width_multiplier=1, cfg={}):
    ...
    if dim == 1:
        if classifier:
            ...
            layers.append(_linear_gn_relu(in_channels, _make_divisible(r * out_channels[-1])))
    else:
        if classifier:
            layers.append(nn.Conv1d(in_channels, out_channels[-1], 1))
        else:
            layers.append(SharedMLP(in_channels, _make_divisible(r * out_channels[-1])))
    return layers, out_channels[-1] if classifier else _make_divisible(r * out_channels[-1])
...
def create_pointnet2_sa_components(sa_blocks, extra_feature_channels, 
    ...
    for conv_configs, sa_configs in sa_blocks:
        ...
        if conv_configs is not None:
            ...
            out_channels = _make_divisible(r * out_channels)
        ...
        if sa_configs is not None:
            ...
            for oc in out_channels:
                if isinstance(oc, (list, tuple)):
                    _out_channels.append([_make_divisible(r * _oc) for _oc in oc])
                else:
                    _out_channels.append(_make_divisible(r * oc))
...
def create_pointnet2_fp_modules(fp_blocks, in_channels, sa_in_channels, embed_dim=64, use_att=False,
    ...
    for fp_idx, (fp_configs, conv_configs) in enumerate(fp_blocks):
        fp_blocks = []
        out_channels = tuple(_make_divisible(r * oc) for oc in fp_configs)
        fp_blocks.append(
            PointNetFPModule(
